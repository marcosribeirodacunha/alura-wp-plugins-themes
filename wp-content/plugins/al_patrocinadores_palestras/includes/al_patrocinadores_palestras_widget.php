...
class PatrocinadoresAlura extends WP_Widget {
  ...
  {
    // A fim de garantir a segurança da nossa aplicação evitando que usuários maliciosos
    // insiram códigos no valor do input, vamos sanitizar o valor retornado
    $instance = array();
    ...
    <section class="patrocinadores-principais">
      ...
      <ul class="lista-patrocinadores">
        <?php if(!empty($instance['caelum'])): ?>
          <li><img src="<?= plugin_dir_url(__FILE__) . '../images/caelum.svg' ?>" alt="Logomarca da Caelum"></li>
        <?php endif; ?>

        <?php if(!empty($instance['casa_do_codigo'])): ?>
          <li><img src="<?= plugin_dir_url(__FILE__) . '../images/cdc.svg' ?>" alt="Logomarca da Casa do Código"></li>
        <?php endif; ?>

        <?php if(!empty($instance['hipsters'])): ?>
          <li><img src="<?= plugin_dir_url(__FILE__) . '../images/hipsters.svg' ?>" alt="Logomarca do Hipsters"></li>
        <?php endif; ?>
      </ul>
    </section>
...
function al_pat_pal_register_widget() {
  register_widget('PatrocinadoresAlura');
}

add_action('widgets_init', 'al_pat_pal_register_widget');
